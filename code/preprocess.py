...
class Preprocess_Sc:
        ...
        if not os.path.exists(o_img) or redo:
            ...
            if img_type=="func":
                if tissue==None:
                    ...
                    cmd=f"sct_qc -i {i_img} -s {csf_mask} -p sct_propseg -qc {self.qc_dir } -v 0" # QC report
        ...
        if verbose:
            if img_type=="func":
                if os.path.exists(o_manual):
                    if redo_qc==True:
                        if tissue==None:
                            cmd_qc=f"sct_qc -i {i_img} -s {o_manual} -p sct_deepseg_sc -qc {self.qc_dir} -v 0"
                        elif tissue=="csf":
                            cmd_qc=f"sct_qc -i {i_img} -s {o_manual} -p sct_propseg -qc {self.qc_dir} -v 0"
                        os.system(cmd_qc)
    ...
    def label_vertebrae(
        ...
        if not os.path.exists(label_file) or redo:
            if auto:
                ...
                cmd = (f"sct_deepseg spine -i {i_img} -o {o_folder} -qc {self.qc_dir} -qc-subject sub-{ID}")
    ...
    def coreg_img2PAM50(self,ID=None,i_img=None,o_folder=None,i_seg=None,PAM50_cord=None,PAM50_t2=None,img_type="func",coreg_type="slicereg",initwarp=None,initwarpinv=None,param=None,ses_name='',task_name='',run_name="",redo=False,verbose=True):
        ...
        if not os.path.exists(o_img) or redo:
            ...
            cmd_coreg=f"sct_register_multimodal -d {PAM50_t2} -dseg {PAM50_cord} -i {i_img} -iseg {i_seg} -param {param} -initwarp {initwarp} -initwarpinv {initwarpinv} -owarp {o_warp_img} -owarpinv {o_warpinv_img} -ofolder {o_folder} -x spline -qc {self.qc_dir} -v 0"
            os.system(cmd_coreg)
